...
The training implements Stage 1 (contrastive pretraining) from the Qwen3-VL-Embedding paper (`qwen-3-vl-emb-paper/colm2024_conference.tex`):
...
python src/eval/run_mmeb.py --model_path <path> --full --output_dir results/mmeb/<name>

# Video tasks (18 tasks)
python src/eval/run_mmeb_video.py --model_path <path> --full

# Visual document tasks (24 tasks)
python src/eval/run_mmeb_visdoc.py --model_path <path> --full
```
...
1. **Direct** via `src/eval/run_mmteb.py` — wraps models in an MTEB-compatible encoder using per-task instructions from `src/eval/task_prompts.json`:
   ```bash
   python src/eval/run_mmteb.py --model_path <path> --sts --output_dir results/mmteb/<name>
   ```
...
- `qwen-3-vl-emb-paper/colm2024_conference.tex` — the paper. Section 5.1 for loss formulations, 5.1.1 for MRL, 5.2 for training setup, Section 4 for multi-stage pipeline.
...
- `qwen-3-vl-emb.md`, `qwen-3.5-0.8b.md` — HuggingFace model cards for reference.
